...
The MVP creates one random `anon_` user ID per device and keeps it in local Hive storage. Existing health-record API calls use that ID instead of the shared `user_test_001` value.
